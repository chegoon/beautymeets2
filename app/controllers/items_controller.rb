...
=end
	...
	def show
		...
		item_ids_in_category = Item.joins(:categories).where("items.published is true AND category_id IN (?) AND items.id <> ?", sibling_categories.map(&:id), @item.id).select("distinct(items.id)")#.order("items.view_count DESC")
