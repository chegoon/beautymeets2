...
=end
	    ...
	    @best_tutorials = Tutorial.where("id IN (?)", Impression.find_by_sql("SELECT impressionable_id 
FROM impressions 
WHERE impressionable_type = 'Tutorial'
...
LIMIT 2").map(&:impressionable_id))
	    ...
	    @best_items = Item.where("id IN (?)", Impression.find_by_sql("SELECT impressionable_id 
FROM impressions 
WHERE impressionable_type = 'Item'
AND (created_at > CURDATE() - INTERVAL 2 WEEK)
GROUP BY impressionable_id 
ORDER BY count(impressionable_id)  DESC
