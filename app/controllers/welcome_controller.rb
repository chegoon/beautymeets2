...
=end
	    ...
	    @best_videos = Video.where(published: true).unread_by(current_user).order("view_count DESC, created_at DESC").limit(4)
	    @latest_videos = Video.where(published: true).unread_by(current_user).order("published_at DESC").limit(4)
	    ...
	    @latest_videos = Video.where(published: true).order("published_at DESC").limit(4)
	    @best_items = Item.order("view_count DESC, created_at DESC").limit(4)
			@latest_items = Item.order("created_at DESC, updated_at DESC").limit(4)
			@latest_posts = Post.order("created_at DESC, updated_at DESC").limit(3)
	  end
